...
class AnalysisController(QtWidgets.QDialog):
    ...
    def __init_ui(self):
        ...
        if self.demo_type == DemoType.HYPERSPECTRAL_DEMO:
            data_gui_stretch = 1
        if self.data_gui:
            # if self.demo_type == DemoType.POINT_DEMO:
            #     self.widget_layout.addWidget(CompositeImageViewer(), 0)
    ...
    def start_embedding(
        ...
        self.embedding_viewer.init_plot(
            ...
            self.on_selection, self.do_close, self.analysis.landmark_orig_indexes,
            disable_select,
